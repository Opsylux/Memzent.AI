  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-8 right-8 w-16 h-16 rounded-full bg-gradient-to-br from-memzent-purple to-memzent-glow flex items-center justify-center text-black shadow-[0_0_30px_rgba(0,243,255,0.3)] hover:scale-110 transition-all z-50 group"
      >
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-32 right-8 w-96 h-[600px] glass bg-memzent-dark/95 border-white/10 rounded-3xl z-50 flex flex-col overflow-hidden shadow-2xl animate-in slide-in-from-bottom-5 duration-300">
          <header className="p-6 border-b border-white/5 bg-white/[0.02] flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-memzent-purple/20 flex items-center justify-center text-memzent-purple">
                <Bot size={18} />
              </div>
              <div>
                <h3 className="text-xs font-black uppercase tracking-widest italic">Neural_Assistant</h3>
                <div className="flex items-center gap-2 mt-0.5">
                  <div className="w-1 h-1 rounded-full bg-memzent-accent animate-pulse" />
            ...
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center ${msg.role === 'user' ? 'bg-memzent-glow/10 text-memzent-glow' : 'bg-white/5 text-white/40'
                    }`}>
          ...
          <form onSubmit={handleSend} className="p-6 border-t border-white/5 bg-white/[0.02]">
            <div className="relative group">
              <input
                ...
                className="w-full h-12 bg-black/40 border border-white/10 rounded-xl pl-4 pr-12 text-xs font-bold text-white focus:border-memzent-glow outline-none transition-all"
              />
              <button
                type="submit"
                disabled={loading || !input.trim()}
                className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-memzent-glow rounded-lg flex items-center justify-center text-black hover:scale-105 transition-all disabled:opacity-30 disabled:scale-100 disabled:grayscale"
              >
                <Send size={14} />
              </button>
            </div>
            <div className="mt-4 flex items-center justify-between text-[9px] font-black uppercase tracking-widest text-white/10 px-1">
              <div className="flex items-center gap-1.5 hover:text-memzent-purple transition-colors cursor-help">
                <Zap size={10} />
                <span>Low Latency</span>
              </div>
              <div className="flex items-center gap-1.5 hover:text-memzent-glow transition-colors cursor-help">
                <ShieldCheck size={10} />
                <span>Encrypted</span>
              </div>
            </div>
