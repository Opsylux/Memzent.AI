...
import { getAuraAudit } from "@/app/actions"
...
export function AuditLogFeed({ orgId }: { orgId?: string }) {
  ...
  useEffect(() => {
    async function fetchAudit() {
      try {
        const data = await getAuraAudit(orgId)
        setEvents(data || [])
      } catch (err) {
  ...
  return (
    <div className="stat-card glow-cyan border-white/10 p-8 neural-bg relative overflow-hidden bg-black/40 h-full flex flex-col">
       ...
       <div className="font-mono text-[9px] space-y-5 flex-1 overflow-y-auto">
         {events.length === 0 ? (
           <div className="text-white/10 italic text-center py-10">Initializing Neural Feed...</div>
         ) : (
           events.map((event, i) => (
             <div key={i} className={`flex gap-4 border-l pl-4 py-1 hover:bg-white/[0.03] transition-colors group ${
               event.status === 'error' ? 'border-red-500/50' : 
               event.status === 'warning' ? 'border-aura-purple/50' : 
               'border-white/5'
             }`}>
                <span className="text-aura-glow/40 font-bold">
                  {new Date(event.timestamp).toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                </span>
                <span className={`transition-colors ${
                  event.status === 'error' ? 'text-red-400' :
                  event.status === 'warning' ? 'text-aura-purple' :
