...
export default function DocsLayout({
  ...
  return (
    <div className="min-h-screen bg-aura-dark flex flex-col">
      {/* Public Docs Header */}
      <header className="h-16 border-b border-white/5 sticky top-0 bg-aura-dark/80 backdrop-blur-md z-50 flex items-center justify-between px-6">
        <div className="flex items-center gap-8">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <Link href="/docs" className="text-xs font-bold text-white uppercase tracking-widest hover:text-aura-glow transition-colors">Documentation</Link>
            <Link href="https://github.com/Opsylux/AuraMCP" target="_blank" className="text-xs font-bold text-white/40 uppercase tracking-widest hover:text-white transition-colors">GitHub</Link>
          </nav>
        </div>
      ...
      <div className="flex-1 flex max-w-7xl mx-auto w-full relative">
        ...
        <main className="flex-1 px-6 lg:px-12 py-10 min-w-0">
          <div className="max-w-3xl prose prose-invert prose-aura">
            {children}
          </div>
          ...
          <footer className="mt-20 pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between gap-6 pb-20">
            <div>
              <div className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20 mb-2 italic">Aura_Neural_Mesh</div>
            ...
            <div className="flex gap-12">
              <div className="flex flex-col gap-3">
                ...
                <Link href="#" className="text-xs font-bold text-white/40 hover:text-aura-glow transition-colors">API Keys</Link>
                <Link href="#" className="text-xs font-bold text-white/40 hover:text-aura-glow transition-colors">RBAC Specs</Link>
              </div>
              <div className="flex flex-col gap-3">
                <span className="text-[10px] font-black uppercase tracking-widest text-white/20">Enterprise</span>
                <Link href="#" className="text-xs font-bold text-white/40 hover:text-aura-glow transition-colors">SLA</Link>
                <Link href="#" className="text-xs font-bold text-white/40 hover:text-aura-glow transition-colors">Security</Link>
              </div>
            </div>
          </footer>
        </main>
      ...
      <div className="fixed bottom-0 right-0 w-[500px] h-[500px] bg-aura-glow/5 blur-[150px] -z-10 rounded-full pointer-events-none" />
    </div>
  );
}
