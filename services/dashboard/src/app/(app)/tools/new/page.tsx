...
import { createAuraTool } from '../../actions'
...
export default function NewToolPage() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      await createAuraTool(orgId, formData)
  ...
  return (
    <div className="flex min-h-screen bg-aura-dark">
      <Sidebar />
      <main className="flex-1 p-10 max-w-4xl mx-auto">
        <header className="mb-12">
          <div className="flex items-center gap-4 mb-4">
             ...
             <Badge className="bg-aura-glow/10 text-aura-glow border-aura-glow/20 px-3 uppercase font-black text-[10px]">Registry v2.0</Badge>
          </div>
          <h1 className="text-4xl font-black tracking-tighter text-white mb-2 italic">
            PROVISION NEW ARMAMENT
          </h1>
          <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">
            Connect external infrastructure to the Aura AI Mesh.
          </p>
        </header>
            ...
            <Card className={`border-white/5 bg-white/5 transition-opacity ${step !== 1 ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
               ...
               <CardContent className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-aura-glow outline-none transition-all"
                  ...
                  <div className="space-y-2 col-span-2">
                    ...
                    <textarea 
                      className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-aura-glow outline-none transition-all h-20 resize-none"
               ...
               {step === 1 && (
                 <CardFooter>
                    <Button onClick={() => setStep(2)} className="bg-white text-black font-black uppercase tracking-widest text-xs px-8 py-6 rounded-2xl hover:bg-aura-glow transition-all flex items-center gap-2">
            ...
            <Card className={`border-white/5 bg-white/5 transition-opacity ${step !== 2 ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
               ...
               <CardContent className="grid grid-cols-2 gap-4">
                  <div 
                    ...
                    className={`p-6 rounded-3xl border cursor-pointer transition-all ${formData.connector_type === 'rest' ? 'border-aura-glow bg-aura-glow/5' : 'border-white/5 hover:border-white/20'}`}
                  >
                    <Globe size={32} className={`mb-4 ${formData.connector_type === 'rest' ? 'text-aura-glow' : 'text-white/20'}`} />
                  ...
                  <div 
                    ...
                    className={`p-6 rounded-3xl border cursor-pointer transition-all ${formData.connector_type === 'sql' ? 'border-aura-accent bg-aura-accent/5' : 'border-white/5 hover:border-white/20'}`}
                  >
                    <Database size={32} className={`mb-4 ${formData.connector_type === 'sql' ? 'text-aura-accent' : 'text-white/20'}`} />
               ...
               {step === 2 && (
                 <CardFooter className="flex justify-between">
                    ...
                    <Button onClick={() => setStep(3)} className="bg-white text-black font-black uppercase tracking-widest text-xs px-8 py-6 rounded-2xl hover:bg-aura-glow transition-all flex items-center gap-2">
                  ...
                  {formData.connector_type === 'rest' ? (
                    <div className="space-y-6">
                      <div className="space-y-2">
                        ...
                        <input 
                          className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm font-mono focus:border-aura-glow outline-none transition-all"
                      ...
                      <div className="p-4 border border-blue-500/20 bg-blue-500/5 rounded-2xl flex gap-4">
                        ...
                        <p className="text-[10px] text-blue-400/80 font-bold leading-relaxed uppercase">
                          Aura will send a POST request to this endpoint with the LLM's parsed arguments in the JSON body.
                        </p>
                      </div>
                    </div>
                  ) : (
                    <div className="space-y-6">
                      <div className="space-y-2">
                        ...
                        <input 
                          type="password"
                          className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm font-mono focus:border-aura-accent outline-none transition-all"
                      ...
                      <div className="space-y-2">
                         ...
                         <textarea 
                           className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-sm font-mono focus:border-aura-accent outline-none transition-all h-24 resize-none"
               ...
               {step === 3 && (
                 <CardFooter className="flex justify-between">
                    ...
                    <Button 
                      ...
                      className="bg-aura-glow text-black font-black uppercase tracking-widest text-xs px-10 py-6 rounded-2xl hover:shadow-[0_0_20px_rgba(0,243,255,0.4)] transition-all flex items-center gap-2"
                    >
        ...
        <section className="mt-12 flex items-center gap-4 p-6 bg-white/[0.02] border border-white/5 rounded-3xl text-sm font-bold opacity-40 italic">
            <Shield size={18} />
            Provisioned tools are isolated to your organization and require valid AURA-SC-01 security clearace for execution.
        </section>
      </main>
    </div>
  )
