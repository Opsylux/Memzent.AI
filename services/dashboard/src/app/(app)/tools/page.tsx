  ...
  return (
    <div className="space-y-12">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="stat-card border-aura-glow/10 shadow-[0_0_20px_rgba(0,243,255,0.02)]">
        ...
        <div className="stat-card border-aura-accent/10">
          ...
          <div className="text-3xl font-black text-aura-accent">EXCELLENT</div>
        </div>
      </section>
      ...
      <section className="stat-card neural-bg border-white/5 p-0 overflow-hidden">
        <div className="p-8 border-b border-white/5 flex flex-col md:flex-row justify-between gap-6">
           <div className="relative group flex-1 max-w-md">
             <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/20 group-hover:text-aura-glow transition-colors" />
             <input 
               type="text" 
               placeholder="Filter Registry..."
               className="w-full h-12 bg-white/[0.03] border border-white/5 rounded-xl pl-12 pr-4 text-xs font-bold tracking-tight focus:outline-none focus:border-aura-glow/20 transition-all"
             />
           </div>
           <div className="flex items-center gap-2">
              <button className="glass h-12 px-4 rounded-xl flex items-center gap-2 text-[10px] font-black uppercase text-white/40 hover:text-white transition-all">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/[0.02] border-b border-white/5 text-[10px] uppercase font-black tracking-widest text-white/40">
                ...
                <tr key={tool.id} className="border-b border-white/5 hover:bg-white/[0.03] transition-all group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                       <div className="w-10 h-10 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-white/20 group-hover:text-aura-glow transition-colors">
                         <Database size={18} />
                       </div>
                       <div>
                         <div className="text-sm font-black tracking-tight group-hover:text-white transition-colors">{tool.name}</div>
                         <div className="text-[10px] font-mono text-white/20 uppercase truncate w-64">{tool.id}</div>
                       </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <span className="text-[10px] font-black px-2 py-1 rounded-md bg-white/5 border border-white/5 uppercase">
                      {tool.provider || 'Aura_Internal'}
                    </span>
                  </td>
                  <td className="px-8 py-6 text-[10px] font-bold">
                    <div className="flex items-center gap-2">
                       <div className="w-1.5 h-1.5 rounded-full bg-aura-accent shadow-[0_0_8px_#00ff8e] animate-pulse" />
                       <span className="text-aura-accent/60 uppercase">ONLINE</span>
                    </div>
                  </td>
                  <td className="px-8 py-6 font-mono text-xs font-black text-white/40 italic">
                    0.992
                  </td>
