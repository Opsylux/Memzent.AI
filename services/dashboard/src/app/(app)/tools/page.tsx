...
export default async function ToolsPage() {
  ...
  const providerCount = new Set((tools || []).map((tool: any) => tool.provider || 'Internal')).size;

  return (
    <div className="space-y-12">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
        <div>
          <h1 className="text-4xl font-black tracking-tighter">Tool Registry</h1>
          <p className="text-sm font-bold text-white/50 uppercase tracking-widest mt-1">
            {org?.orgName ? `${org.orgName} — ` : ''}MCP Tool Configuration &amp; Routing
          </p>
        </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="stat-card border-memzent-glow/10 shadow-[0_0_20px_rgba(0,243,255,0.02)]">
          <div className="text-[10px] font-black text-white/50 uppercase mb-1">Total Registered</div>
        ...
        <div className="stat-card border-memzent-accent/10">
          ...
          <div className="text-3xl font-black text-memzent-accent">HEALTHY</div>
        </div>
      </section>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/[0.02] border-b border-white/5 text-[10px] uppercase font-black tracking-widest text-white/60">
                ...
                <th className="px-8 py-6">Connector</th>
                ...
                <tr key={tool.id} className="border-b border-white/5 hover:bg-white/[0.03] transition-all group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] font-mono text-white/45 truncate max-w-[250px]">{tool.description || tool.id}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                  ...
                  <td className="px-8 py-6">
                    <span className="text-[10px] font-bold text-white/60 uppercase">
                      {tool.connector_type || 'mcp'}
                    </span>
                  </td>
                  <td className="px-8 py-6 text-[10px] font-bold">
                    <div className="flex items-center gap-2">
                      <div className={`w-1.5 h-1.5 rounded-full ${tool.status === 'online' ? 'bg-memzent-accent shadow-[0_0_8px_#00ff8e]' : 'bg-yellow-500'} animate-pulse`} />
                      <span className={`uppercase ${tool.status === 'online' ? 'text-memzent-accent/90' : 'text-yellow-500/90'}`}>
                        {tool.status || 'online'}
                      </span>
                    </div>
                  </td>
                  <td className="px-8 py-6 text-right">
                    <ToolActions tool={tool} orgId={org?.orgId} />
                  </td>
                </tr>
              ))}
            </tbody>
